...
Use case diagram pertama menggambarkan interaksi User Unit dengan sistem yang mencakup fungsi Login, Reset Password via Token,
Input Laporan Harian, Input Target/RKAP, Validasi Internal Laporan,
Submit Laporan, Export Laporan PDF/Excel, Ajukan Helpdesk TOKEN, Ajukan Helpdesk ISSUE, dan Ajukan Helpdesk REVISION. Terdapat dua relasi tambahan pada diagram ini, yaitu fungsi Tambah Komoditi Barang yang bersifat «extend» terhadap Input Laporan Harian, artinya fitur ini hanya digunakan oleh User Unit Angkutan Barang saat diperlukan, serta fungsi Lihat Status Laporan yang bersifat «extend» terhadap Submit 
Laporan. Selain itu, fungsi Validasi Internal Laporan memiliki relasi «include» terhadap Submit Laporan, yang berarti proses validasi internal wajib dilalui sebelum laporan dapat di-submit. Use case diagram aktor User Unit dapat dilihat pada Gambar \ref{fig:3.usecase-user-unit} berikut.  
...
\end{figure}
Use case diagram kedua menggambarkan interaksi Admin Global dengan sistem yang mencakup fungsi Login, Export Laporan 
PDF/Excel, Review Laporan, Monitor Semua Unit, Kelola Akun User, Kelola Unit, dan Lihat Dashboard \& Grafik. Fungsi Review Laporan memiliki dua relasi «include», yaitu ACC Laporan dan Set Status REVISI, yang berarti kedua fungsi tersebut selalu menjadi bagian dari proses review yang dilakukan Admin Global. Fungsi Handle Helpdesk REVISION memiliki relasi «extend» terhadap Set Status REVISI, artinya penetapan status REVISI juga dapat dipicu melalui jalur penanganan tiket helpdesk dari User Unit. Use case diagram aktor Admin Global dapat dilihat pada Gambar \ref{fig:3.usecase-admin-global} berikut.  

Use case diagram ketiga menggambarkan interaksi IT Support dengan sistem yang mencakup fungsi Login, Monitor Semua Unit, 
Handle Helpdesk TOKEN, dan Handle Helpdesk ISSUE. Fungsi Handle 
Helpdesk TOKEN memiliki relasi «include» terhadap Generate Token Reset Password, yang berarti setiap kali IT Support menangani tiket TOKEN, pembangkitan token reset password selalu menjadi bagian dari proses penanganannya. Use case diagram aktor IT Support dapat dilihat pada Gambar \ref{fig:3.usecase-it-support} berikut.  
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.70\textwidth]{figure/activity_png/activity_1_login.drawio.png}
    \caption{\textit{Activity Diagram} 1: Login \& Mekanisme Lock Akun}
    \label{fig:3.ad-login}
\end{figure}

Alur proses login beserta mekanisme penguncian akun ini dapat dilihat pada Gambar \ref{fig:3.ad-login} berikut.   
...
Alur lengkap proses reset password melalui token ini dapat dilihat pada Gambar \ref{fig:3.ad-reset} berikut. \par
...
\textit{User Acceptance Testing} (UAT) dilakukan untuk mengukur tingkat keberterimaan sistem oleh pengguna akhir berdasarkan standar ISO/IEC 25010 \cite{lamada2024iso}. Pengujian ini melibatkan responden yang dipilih menggunakan teknik \textit{purposive sampling} \cite{sugiyono2019} berdasarkan kriteria keterlibatan langsung pengguna dalam proses operasional sistem pelaporan kinerja harian, dengan total responden berkisar antara 16 hingga 20 orang yang terbagi ke dalam tiga kelompok peran, yaitu User Unit, Admin Global, dan IT Support di lingkungan PT KAI Divre I Sumatera Utara. \par

Aspek-aspek yang diuji mencakup enam karakteristik kualitas ISO/IEC 25010, yaitu Keamanan (\textit{Security}), Efisiensi Kinerja (\textit{Performance Efficiency}), Kemudahan Penggunaan (\textit{Usability}), Keandalan (\textit{Reliability}), Kompatibilitas (\textit{Compatibility}), dan Ketersediaan (\textit{Availability}) \cite{lamada2024iso}. Instrumen pengujian menggunakan kuesioner dengan skala Likert 1--5 dengan kriteria penilaian sebagai berikut: (1) Sangat Tidak Setuju, (2) Tidak Setuju, (3) Netral, (4) Setuju, dan (5) Sangat Setuju. \par

Sistem dinyatakan diterima (\textit{accepted}) apabila hasil persentase keberterimaan secara keseluruhan mencapai nilai minimal 61\% atau berada pada kategori Layak hingga Sangat Layak. Rancangan kuesioner UAT berdasarkan standar ISO/IEC 25010 dapat dilihat pada Tabel \ref{table:3.uat} berikut: \par
